...
if (-not (Get-Command uv -ErrorAction SilentlyContinue)) {
  Write-Host "installing uv..."; powershell -c "irm https://astral.sh/uv/install.ps1 | iex"
  $env:PATH = "$env:USERPROFILE\.local\bin;$env:PATH"
}
try { uv tool install "git+https://github.com/oraios/serena" ; Write-Host "serena installed" }
catch { Write-Warning "serena install failed: $($_.Exception.Message)" }
...
if (Get-Command claude -ErrorAction SilentlyContinue) {
  ...
  } catch { Write-Warning "‘claude mcp add’ failed — add serena manually (see ONBOARDING.md §2)." }
} else {
...
Write-Host " - Install az CLI + Terraform if working on Azure/IaC (see ONBOARDING.md §0/§3)."
Write-Host " - az login (note the MFA/WAM-broker gotcha in ONBOARDING.md §3)."
